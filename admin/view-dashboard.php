...
<div class="wrap">
    ...
    <div style="background: #fff; border: 1px solid #ccd0d4; border-left: 4px solid <?php echo ( $licence_status === 'authorised' ) ? '#46b450' : '#d63638'; ?>; padding: 15px 20px; margin-bottom: 20px; border-radius: 4px; display: flex; align-items: center; justify-content: space-between;">
        ...
        <form method="post" action="" style="display: flex; gap: 10px; align-items: center;">
            <?php wp_nonce_field( 'cit_licence_action', 'cit_licence_nonce' ); ?>
            <?php if ( $licence_status !== 'authorised' ) : ?>
                <input type="hidden" name="cit_licence_action_type" value="activate" />
                <input type="text" name="cit_licence_key" value="<?php echo esc_attr( $licence_key ); ?>" placeholder="ITPRO-XXXXX-XXXXX" style="padding: 6px 10px; font-size: 13px; width: 220px; border: 1px solid #8c8f94; border-radius: 4px;" />
                <input type="submit" name="cit_licence_submit" class="button button-primary" value="Activate Licence" />
            <?php else : ?>
                <input type="hidden" name="cit_licence_action_type" value="deactivate" />
                ...
                <input type="submit" name="cit_licence_submit" class="button button-secondary" value="Deactivate" />
            <?php endif; ?>
        </form>
    </div>
    ...
    <nav class="nav-tab-wrapper">
        <a href="?page=cit-search-dashboard&tab=dashboard" class="nav-tab <?php echo ($active_tab === 'dashboard') ? 'nav-tab-active' : ''; ?>">🔍 Dashboard & Search Log</a>
        <a href="?page=cit-search-dashboard&tab=popout_adverts" class="nav-tab <?php echo ($active_tab === 'popout_adverts') ? 'nav-tab-active' : ''; ?>">📢 Dynamic Adverts</a>
        <a href="?page=cit-search-dashboard&tab=design_customiser" class="nav-tab <?php echo ($active_tab === 'design_customiser') ? 'nav-tab-active' : ''; ?>">🎨 Styling Customiser</a>
        <a href="?page=cit-search-dashboard&tab=keyword_scanner" class="nav-tab <?php echo ($active_tab === 'keyword_scanner') ? 'nav-tab-active' : ''; ?>">🤖 Scanner & Dictionary</a>
        <a href="?page=cit-search-dashboard&tab=access_control" class="nav-tab <?php echo ($active_tab === 'access_control') ? 'nav-tab-active' : ''; ?>">🔒 Access Control</a>
    </nav>
    ...
    if ( $active_tab === 'dashboard' ) : 
        ...
        if ( isset($_POST['cit_purge_logs']) && current_user_can('manage_options') ) {
            check_admin_referer('cit_purge_logs_action', 'cit_purge_logs_nonce');
        ...
        <div style="background: #fff; padding: 25px; margin-top: 15px; border: 1px solid #ccd0d4; border-radius: 4px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px;">
                ...
                <form method="post" action="" onsubmit="return confirm('Purge search feedback?');">
                    <?php wp_nonce_field('cit_purge_logs_action', 'cit_purge_logs_nonce'); ?>
                    <input type="submit" name="cit_purge_logs" class="button button-link-delete" value="Purge Audit Log Data" style="color:#d63638; text-decoration:none;" />
                </form>
            </div>
    ...
    elseif ( $active_tab === 'popout_adverts' ) : 
        if ( isset($_POST['cit_save_adverts']) && current_user_can('manage_options') ) {
            check_admin_referer('cit_save_adverts_action', 'cit_save_adverts_nonce');
            
            $settings = isset($_POST['cit_advert_settings']) ? $_POST['cit_advert_settings'] : [];
            update_option('cit_advert_settings', $settings);
        ...
        $g_title = esc_attr($opts['global']['main_title'] ?? 'Welcome to The Andersons Centre');
        $g_url   = esc_url($opts['global']['main_url'] ?? '/store/');
        $g_btn   = esc_attr($opts['global']['main_btn'] ?? 'Find Out More');
        ?>
        <form method="post" action="">
            <?php wp_nonce_field('cit_save_adverts_action', 'cit_save_adverts_nonce'); ?>
            ...
            <div style="background:#fff; padding:25px; border:2px solid #e1ad01; margin: 15px 0 30px 0; border-radius:4px;">
                ...
                <table class="form-table" style="margin-top:10px;">
                    <tr>
                        ...
                        <td><input type="text" name="cit_advert_settings[global][main_title]" value="<?php echo $g_title; ?>" class="large-text" /></td>
                    </tr>
                    <tr>
                        <th><label>Destination Hyperlink URL</label></th>
                        <td><input type="text" name="cit_advert_settings[global][main_url]" value="<?php echo $g_url; ?>" class="large-text" /></td>
                    </tr>
                    <tr>
                        <th><label>Link Call To Action Text</label></th>
                        <td><input type="text" name="cit_advert_settings[global][main_btn]" value="<?php echo $g_btn; ?>" class="large-text" style="max-width:250px;" /></td>
                    </tr>
                </table>
            </div>
            ...
            foreach ($groups as $slug => $label) {
                ...
                <div style="background:#fff; padding:20px; border:1px solid #ccd0d4; margin-bottom:20px; border-radius:4px;">
                    ...
                    <input type="number" name="cit_advert_settings[<?php echo esc_attr($slug); ?>][id]" value="<?php echo $id; ?>" style="width:100%; max-width:200px; margin-top:5px;" placeholder="e.g. 412" /></label></p>
                    ...
                    <div style="display:flex; gap:20px; margin-top:15px;">
                        <div style="flex:1; background:#f9f9f9; padding:15px; border:1px solid #eee; border-radius:3px;">
                            ...
                            <p><label>Title Text:<br/><input type="text" name="cit_advert_settings[<?php echo esc_attr($slug); ?>][main_title]" value="<?php echo $m_title; ?>" style="width:100%;" /></label></p>
                            <p><label>Description Text:<br/><textarea name="cit_advert_settings[<?php echo esc_attr($slug); ?>][main_desc]" rows="3" style="width:100%; margin-top:5px;"><?php echo $m_desc; ?></textarea></label></p>
                            <p><label>Destination URL Link:<br/><input type="text" name="cit_advert_settings[<?php echo esc_attr($slug); ?>][main_url]" value="<?php echo $m_url; ?>" style="width:100%;" /></label></p>
                            <p><label>Action Button Text:<br/><input type="text" name="cit_advert_settings[<?php echo esc_attr($slug); ?>][main_btn]" value="<?php echo $m_btn; ?>" style="width:100%;" /></label></p>
                        </div>
                        
                        <div style="flex:1; background:#f9f9f9; padding:15px; border:1px solid #eee; border-radius:3px;">
                            <h4 style="margin-top:0; color:#646970;">🔄 Alternative Ad Line (Already Bought Product ID)</h4>
                            <p><label>Alt Title Text:<br/><input type="text" name="cit_advert_settings[<?php echo esc_attr($slug); ?>][alt_title]" value="<?php echo $a_title; ?>" style="width:100%;" /></label></p>
                            <p><label>Alt Description Text:<br/><textarea name="cit_advert_settings[<?php echo esc_attr($slug); ?>][alt_desc]" rows="3" style="width:100%; margin-top:5px;"><?php echo $a_desc; ?></textarea></label></p>
                            <p><label>Alt Destination URL:<br/><input type="text" name="cit_advert_settings[<?php echo esc_attr($slug); ?>][alt_url]" value="<?php echo $a_url; ?>" style="width:100%;" /></label></p>
                            <p><label>Alt Action Button Text:<br/><input type="text" name="cit_advert_settings[<?php echo esc_attr($slug); ?>][alt_btn]" value="<?php echo $a_btn; ?>" style="width:100%;" /></label></p>
                        </div>
                    </div>
                </div>
                <?php
            ...
            ?>
            <p class="submit" style="padding-top:10px;"><input type="submit" name="cit_save_adverts" class="button button-primary button-large" value="Save All Advert Configurations" /></p>
        </form>
    ...
    elseif ( $active_tab === 'design_customiser' ) : 
        if ( isset($_POST['cit_save_design']) && current_user_can('manage_options') ) {
            check_admin_referer('cit_save_design_action', 'cit_save_design_nonce');
            
            $design = isset($_POST['cit_design_settings']) ? array_map('sanitize_text_field', $_POST['cit_design_settings']) : [];
            update_option('cit_design_settings', $design);
        ...
        ?>
        <form method="post" action="">
            <?php wp_nonce_field('cit_save_design_action', 'cit_save_design_nonce'); ?>
            <div style="background:#fff; padding:25px; border:1px solid #ccd0d4; border-radius:4px; margin-top: 15px;">
                ...
                <table class="form-table" style="margin-top: 20px;">
                    <tr>
                        ...
                        <td>
                            <input type="color" name="cit_design_settings[heading]" value="<?php echo $c_heading; ?>" style="vertical-align: middle; width:50px; height:30px; padding:0; cursor:pointer;" />
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="color" name="cit_design_settings[recommended]" value="<?php echo $c_recommended; ?>" style="vertical-align: middle; width:50px; height:30px; padding:0; cursor:pointer;" />
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="color" name="cit_design_settings[button]" value="<?php echo $c_button; ?>" style="vertical-align: middle; width:50px; height:30px; padding:0; cursor:pointer;" />
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="color" name="cit_design_settings[button_text]" value="<?php echo $c_button_text; ?>" style="vertical-align: middle; width:50px; height:30px; padding:0; cursor:pointer;" />
                ...
                <p class="submit" style="margin-top: 25px; padding-top: 15px; border-top: 1px solid #eee;">
                    <input type="submit" name="cit_save_design" class="button button-primary button-large" value="Save Interface Styles" />
                </p>
            </div>
        </form>
    ...
    elseif ( $active_tab === 'keyword_scanner' ) : 
        $master_groups = function_exists('lee_dev_get_active_categories_5921') ? lee_dev_get_active_categories_5921() : [];

        if ( $_SERVER['REQUEST_METHOD'] === 'POST' && current_user_can('manage_options') ) {
            
            if ( isset($_POST['cit_trigger_scan']) ) {
                check_admin_referer('cit_scanner_action', 'cit_scanner_nonce');
                ...
                if ( ! empty($raw_posts) ) {
                    foreach ( $raw_posts as $post_obj ) {
                        ...
                        $assigned_cats = [];
                        $post_terms = wp_get_object_terms( $post_obj->ID, array( 'category', 'product_cat' ) );
                ...
                $terms = get_terms(['taxonomy' => array_values( $taxonomies ), 'hide_empty' => false, 'suppress_filters' => true]);
                ...
                if ( ! is_wp_error($terms) && ! empty($terms) ) {
                    foreach ( $master_groups as $cat_slug => $cat_label ) {
                        ...
                        foreach ( $terms as $term ) {
                            ...
                            if ( strlen($clean_slug_phrase) < 3 || in_array($clean_slug_phrase, $blacklist, true) ) continue;
                ...
                update_option('cit_dynamic_keyword_dictionary', $clean_rebuilt_dict);
            ...
            } elseif ( isset($_POST['cit_save_dictionary']) ) {
                check_admin_referer('cit_scanner_action', 'cit_scanner_nonce');
                
                $raw_inputs = isset($_POST['cit_dict']) ? $_POST['cit_dict'] : [];
                $new_dict = [];
                ...
                update_option('cit_dynamic_keyword_dictionary', $new_dict);
        ...
        <div style="background:#fff; padding:25px; border:1px solid #ccd0d4; border-radius:4px; margin-top:15px;">
            ...
            <form method="post" action="" style="margin:20px 0;">
                <?php wp_nonce_field('cit_scanner_action', 'cit_scanner_nonce'); ?>
                <div style="background:#f0f6fa; padding:15px; border-left:4px solid #2271b1; border-radius:3px; display:flex; align-items:center; justify-content:space-between;">
                    ...
                    <input type="submit" name="cit_trigger_scan" class="button button-secondary" value="Scan Website Structure Now" />
                </div>
            </form>
            ...
            <form method="post" action="">
                    ...
                    ?>
                        <tr style="border-top:1px solid #eee;">
                            ...
                            <td style="padding:15px 0;">
                                <textarea name="cit_dict[<?php echo esc_attr($key); ?>]" rows="3" class="large-text" style="font-family:monospace; font-size:13px;" placeholder="e.g. farm management pocketbook, nix, sfi schemes"><?php echo esc_textarea($current_phrases); ?></textarea>
                ...
                <p class="submit" style="margin-top:20px; border-top:1px solid #eee; padding-top:15px;">
                    <input type="submit" name="cit_save_dictionary" class="button button-primary button-large" value="Save Keyphrase Dictionaries" />
                </p>
            </form>
        </div>
    ...
    elseif ( $active_tab === 'access_control' ) : 
        if ( function_exists('lee_dev_render_access_control_tab_content_9281') ) {
            ...
            <div style="background: #fff; padding: 25px; margin-top: 15px; border: 1px solid #ccd0d4; border-radius: 4px; max-width: 650px;">
                ...
                <form method="post" action="">
                    <?php wp_nonce_field( 'cit_save_access_settings_nonce' ); ?>
                                ...
                                <?php foreach ( $wp_roles as $role_slug => $role_name ) : ?>
                                    <div style="margin-bottom: 14px;">
                                        <label style="font-size:14px; display:inline-flex; align-items:center; cursor:pointer;">
                                            <input type="checkbox" name="cit_roles[]" value="<?php echo esc_attr( $role_slug ); ?>" <?php checked( in_array( $role_slug, $allowed_roles, true ) ); ?> style="margin-right:10px;" />
                    ...
                    <p class="submit" style="margin: 0;"><input type="submit" name="cit_save_access_settings" class="button button-primary button-large" value="Update Roles" /></p>
                </form>
            </div>
            <?php
        }
