...
( function() {
	...
	function setProgress( root, isVisible, label ) {
		...
		if ( removeBtn ) { removeBtn.disabled = !! isVisible; }
	}
	...
	function replaceSummary( root, html, hasFaqs ) {
		...
		if ( btn ) {
			btn.textContent = hasFaqs
				? ( BOOT.i18n && BOOT.i18n.regenerate ? BOOT.i18n.regenerate : 'Regenerate FAQs' )
				: ( BOOT.i18n && BOOT.i18n.generate   ? BOOT.i18n.generate   : 'Scan page & generate FAQs' );
		}
	}
	...
	function callRest( path, method, body ) {
		...
		if ( ! apiFetch ) {
			return Promise.reject( new Error( 'wp.apiFetch is not available on this screen.' ) );
		}
		return apiFetch( {
			path:    path,
			method:  method || 'POST',
		...
		callRest( 'intenttargetpro/v1/generate-faqs', 'POST', { post_id: postId } )
			.then( function( response ) {
				...
				if ( ! response || response.success !== true ) {
					var msg = ( response && response.message ) ? response.message : ( BOOT.i18n.errorGeneric || 'Unknown error' );
					showError( root, msg );
					return;
				}
			...
			.catch( function( err ) {
				setProgress( root, false );
				var msg = ( err && err.message ) ? err.message : ( BOOT.i18n.errorGeneric || 'Unknown error' );
