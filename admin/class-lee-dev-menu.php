...
function lee_dev_admin_display_interests_1938($user) {
    ...
    ?>
    <div class="cit-intelligence-profile-wrapper">
...
function lee_dev_reposition_profile_intelligence_panel_4721() {
    ?><script>jQuery(document).ready(function($) { var p = $('.cit-intelligence-profile-wrapper'); if(p.length) p.insertAfter('.wrap h1:first'); });</script><?php
}
...
function lee_dev_show_engagement_status_3847($user) {
    $is_high_engaged = get_user_meta($user->ID, 'cit_high_engagement_flag', true);
    $last_interaction = get_user_meta($user->ID, 'cit_last_interaction_date', true);
    ?>
...
function lee_dev_add_lead_status_column_5829($columns) {
    $columns['cit_lead_status'] = 'Lead Status';
    return $columns;
}
...
function lee_dev_fill_lead_status_column_2940($output, $column_name, $user_id) {
    if ($column_name !== 'cit_lead_status') return $output;
    $is_high_engaged = get_user_meta($user_id, 'cit_high_engagement_flag', true);
    $total_score = get_user_meta($user_id, 'cit_total_score', true) ?: 0;
...
function lee_dev_style_user_table_columns_4812() {
    echo '<style>table.fixed { table-layout: auto !important; } .column-cit_lead_status { min-width: 150px !important; width: 150px; } .column-cit_lead_status span { display: inline-block; text-align: center; min-width: 100px; }</style>';
}

add_action('restrict_manage_users', 'lee_dev_add_lead_status_filter_9381');
function lee_dev_add_lead_status_filter_9381($which) {
    if ($which !== 'top') return;
    $selected = isset($_GET['cit_lead_filter']) ? $_GET['cit_lead_filter'] : '';
    ?>
    <select name="cit_lead_filter" style="float:none; margin-left:10px;">
...
function lee_dev_filter_users_by_lead_status_3819($query) {
    if (!is_admin()) return;
    $filter_value = isset($_GET['cit_lead_filter']) ? $_GET['cit_lead_filter'] : '';
    if (empty($filter_value)) return;

    if ($filter_value === 'hot') {
        $query->set('meta_key', 'cit_high_engagement_flag');
        $query->set('meta_value', 'yes'); 
    } elseif ($filter_value === 'warm') {
        $query->set('meta_key', 'cit_total_score');
        ...
        $query->set('meta_query', array(
            'relation' => 'AND',
            array('key' => 'cit_high_engagement_flag', 'compare' => 'NOT EXISTS'),
            array(
                'relation' => 'OR',
                array('key' => 'cit_total_score', 'value' => '100', 'compare' => '<', 'type' => 'NUMERIC'),
                array('key' => 'cit_total_score', 'compare' => 'NOT EXISTS')
            )
        ));
    }
}
